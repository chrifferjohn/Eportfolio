...
if(isset($_POST['sub'])){
	...
	switch ($volume) {
		case 'cylinder':
			$cylinder = $pi * ($r * $r) * $h;
			echo"Cylinder is equal to ".$cylinder;
			break;
		case 'cube':
			$cube = $l * $l * $l;
			echo"Cube is equal to ".$cube;
			break;
	    case 'sphere':
			$sphere = (4/3) * $pi *($r*$r);
			echo "Sphere is equal to ".$sphere;
			break;
		case 'pyramid':
			$pyramid = (1/3) * $b * $h;
			echo "Pyramid is equal to ".$pyramid;
			break;
		case 'ellipsiod':
			$ellipsiod = (4/3) * $pi * $a * $b * $c;
			echo"Ellipsiod is equal to ".$ellipsiod;
			break;
		default:
			echo"invalid";
			break;
